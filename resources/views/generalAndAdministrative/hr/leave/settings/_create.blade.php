...
<div class="card">
	<div class="card-header">Add New Staff Annual Leave, Medical Certificate Leave And Maternity Leave.</div>
	<div class="card-body">

		<div class="form-group row {{ $errors->has('staff_id') ? ' has-error' : '' }}">
			{{ Form::label('sid', 'Staff : ', ['class' => 'col-sm-2 col-form-label']) }}
			<div class="col-sm-10">
				<select name="staff_id" id="sid" class="form-control">
...
@endforeach
		...
		<div class="form-group row {{ $errors->has('year') ? ' has-error' : '' }}">
			...
			<div class="col-sm-10">
				{{ Form::text('year', $n->copy()->addYear()->year, ['class' => 'form-control', 'id' => 'year', 'placeholder' => 'Year', 'autocomplete' => 'off', 'disabled']) }}
				{{ Form::hidden('year', $n->copy()->addYear()->year) }}
			</div>
		</div>
