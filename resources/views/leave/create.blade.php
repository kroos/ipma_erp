...
if (empty($sel)) {	// apa sebab empty? dalam department DAN di location, dia sorang saja..
	$hj1 = \App\Model\Staff::where('active', 1)->where('location_id', $userloc)->where('leave_need_backup', 1)->get();
	foreach($hj1 as $mn) {
		// echo $mn->belongtomanyposition()->wherePivot('main', 1)->first()->group_id.', ';
		if($mn->belongtomanyposition()->wherePivot('main', 1)->first()->group_id == 4) {	// dapatkan supervisor
			// echo $mn->name.' |';
			$sel[$mn->id] = $mn->name;
		}
	}
