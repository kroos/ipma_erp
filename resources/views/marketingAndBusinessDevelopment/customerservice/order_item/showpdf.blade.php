...
$induk1 = $csOrder->hasmanyorderitem()->get();
// $induk = $induk1->belongtoorder;
...
// Instanciation of inherited class
	...
	$pdf->Cell(30, 5, Carbon::parse($csOrder->date)->format('D, j F Y'), 0, 0, 'L');

	$pdf->SetX(115);
	$pdf->SetFont('Arial', NULL, 9);
	$pdf->Cell(30, 5, 'Delivery Date:', 0, 0, 'L');
	$pdf->SetFont('Arial', 'B', 9);
	$pdf->Cell(30, 5, 'delivery date', 0, 1, 'L');
	...
	$pdf->SetFont('Arial', 'B', 9);
	$pdf->Cell(30, 5, $csOrder->belongtocustomer->customer, 0, 0, 'L');

	$pdf->SetX(115);
	$pdf->SetFont('Arial', NULL, 9);
	$pdf->Cell(30, 5, 'Order By:', 0, 0, 'L');
	$pdf->SetFont('Arial', 'B', 9);
	$pdf->Cell(30, 5, $csOrder->requester, 0, 1, 'L');

	$pdf->SetX(115);
	$pdf->SetFont('Arial', NULL, 9);
	$pdf->Cell(30, 5, 'Customer PO No:', 0, 0, 'L');
	$pdf->SetFont('Arial', 'B', 9);
	$pdf->Cell(30, 5, $csOrder->customer_PO_no, 0, 1, 'L');
	...
	$pdf->Cell(30, 5, 'COP-'.$csOrder->id, 0, 1, 'L');
	...
	$pdf->SetFont('Arial', 'B', 9);
	$pdf->SetWidths([10, 60, 15, 20, 20, 20, 22, 23]);
	...
	foreach($induk1 as $it) {
		...
		$pdf->Row([
			...
			$it->description,
			Carbon::parse($it->delivery_date)->format('D, j F Y'),
			$it->belongtoorderdelivery->delivery_method,
			$it->delivery_remarks
		]);
	...
	$pdf->Cell(0, 10, $csOrder->description, 'BR', 1, 'C');
	...
	$filename = 'Customer Order Parts COP-'.$csOrder->id.'.pdf';
